...
uuid=$(grep -E "^###vls " "/etc/xray/config.json" | cut -d ' ' -f 7-9 | sed -n "${CLIENT_NUMBER}"p)
exp=$(grep -E "^###vls " "/etc/xray/config.json" | cut -d ' ' -f 3-4 | sed -n "${CLIENT_NUMBER}"p)
now=$(date +%Y-%m-%d)
d1=$(date -d "$exp" +%s)
d2=$(date -d "$now" +%s)
exp2=$(( (d1 - d2) / 86400 ))
...
NUMBER_OF_CLIENTS=$(grep -c -E "^###vls " "/etc/xray/config.json")
    ...
    echo "  User       Expired  " 
	echo -e "\033[0;34m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
	grep -E "^###vls " "/etc/xray/config.json" | cut -d ' ' -f 2-4 | column -t | sort | uniq | lolcat
    echo ""
...
harini=`date -d "0 days" +"%Y-%m-%d %T"`
sed -i '/#vless$/a\###vlstrial '"$user $exp $harini $uuid"'\
},{"id": "'""$uuid""'","email": "'""$user""'"' /etc/xray/config.json
sed -i '/#vlessgrpc$/a\###vlstrial '"$user $exp $harini $uuid"'\
